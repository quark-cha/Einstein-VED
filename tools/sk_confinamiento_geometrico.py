...
def crear_animacion_confinamiento():
    ...
    line, = ax.plot([], [], 'b-', linewidth=3, label='Vlna ψ(x)')
    
    # Ejes y etiquetas iniciales
    ax.set_xlabel('Poloha x', fontsize=12)
    ax.set_ylabel('Amplitúda ψ(x)', fontsize=12)
    ax.set_title('EINSTEIN-VED UVÄZNENIE: Vlna → Častica', fontweight='bold', fontsize=14)
    ...
    def animate(frame):
        ...
        if frame < 30:  # FASE 1: Onda completa en papel
            ...
            ax.set_xlabel('Poloha x', fontsize=12)
            ax.set_ylabel('Amplitúda ψ(x)', fontsize=12)
            ax.set_title('1. Sínusoidálna vlna na plochej ploche', fontweight='bold')
            ...
            ax.set_title(f'2. Prechod: Zvinovanie ({progress*100:.0f}%)', fontweight='bold')
            ax.set_xlabel('X súradnica')
            ax.set_ylabel('Y súradnica')
            ...
            ax.plot(x_wave, y_wave, 'red', linewidth=3, label=f'Uväznená (n={n_cycles})')
            ax.text(0.4, -1.8, f'EINSTEIN-VED PODMIENKA: L = {n_cycles}λ', 
                    fontweight='bold', fontsize=12,
                    bbox=dict(boxstyle="round", facecolor="yellow"))
            ax.set_title('3. Samo-uväznená vlna - stabilná častica', fontweight='bold')
            ax.set_xlabel('X súradnica')
            ax.set_ylabel('Y súradnica')
            ax.legend()
...
def generar_resumen_confinamiento():
    ...
    ax1.set_title('a) Voľná rovinná vlna', fontweight='bold')
    ax1.set_xlabel('Poloha x')
    ax1.set_ylabel('Amplitúda ψ(x)')
    ...
    ax2.set_title('b) Prechod', fontweight='bold')
    ax2.set_xlabel('X súradnica')
    ax2.set_ylabel('Y súradnica')
    ...
    ax3.set_title('c) Uväznená n=3\nL = 3λ', fontweight='bold')
    ax3.set_xlabel('X súradnica')
    ax3.set_ylabel('Y súradnica')
    ax3.grid(True, alpha=0.3)
    ax3.text(1.6, -1.3, 'PODMIENKA: L = nλ', ha='center', fontweight='bold',
             bbox=dict(boxstyle="round", facecolor="yellow"))

    plt.tight_layout()
    plt.savefig(r"C:\Users\vedq\Desktop\desarrollo\SRC-VED\Einstein-VED\img\SK_confinamiento.svg", format='svg', bbox_inches='tight')
    print("SVG resumen guardado: 'resumen_confinamiento.svg'")

# Ejecutar
print("Generovanie SVG animácie uväznenia...")
crear_animacion_confinamiento()
generar_resumen_confinamiento()
print("✅ SVG animácia dokončená")
