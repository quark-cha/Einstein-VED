...
# ===========================
idioma_actual = "el"   # definido por el generador del temporal
...
ax1.plot(theta, r, linestyle='--', color='blue', label=r'Ακτίνα πρωτονίου (μεσαία γραμμή)')
for k in range(1, n_onda+1):
    ax1.plot(theta, r*(k/n_onda), color='red', linestyle='-', alpha=0.8)
ax1.set_title(r'Σχήμα 1: Περιορισμένο κύμα VED του πρωτονίου, n=2', pad=20)
ax1.legend()
plt.savefig('C:\\Users\\vedq\\Desktop\\desarrollo\\SRC-VED\\Einstein-VED\\img\\EL_figura_1_proton.png', dpi=300)
# plt.show()  # Desactivado en traducciones
plt.close()
print("Figura 1 generada."
      )
# ===========================
# 4. Figura 2: Μαύρη τρύπα y superficie 2D
# ===========================
R_s = 2 * G_exp * M_BH / c**2

plt.figure(figsize=(6,6))
ax2 = plt.gca()
circle = plt.Circle((0,0), R_s, color='black', alpha=0.7, label=r'Μαύρη τρύπα')
ax2.add_artist(circle)
circle_surf = plt.Circle((0,0), R_s, color='yellow', fill=False, linewidth=2, label=r'2Δ επιφάνεια')
ax2.add_artist(circle_surf)
ax2.set_xlim(-R_s*1.2, R_s*1.2)
ax2.set_ylim(-R_s*1.2, R_s*1.2)
ax2.set_aspect('equal', 'box')
plt.title(r'Figura 2: Μαύρη τρύπα y superficie 2D holográfica', pad=20)
plt.legend()
plt.savefig('C:\\Users\\vedq\\Desktop\\desarrollo\\SRC-VED\\Einstein-VED\\img\\EL_figura_2_proton.png', dpi=300)
...
plt.axhline(G_exp, color='black', linestyle='--', label=r'Πειραματική G')
...
plt.ylabel(r'Υπολογισμένο G [m³/kg/s²]')
plt.title(r'Figura 3: Diana de G $\pm 3\sigma$', pad=20)
plt.xticks(np.arange(0,6))
plt.grid(True)
plt.legend()
plt.savefig('C:\\Users\\vedq\\Desktop\\desarrollo\\SRC-VED\\Einstein-VED\\img\\EL_figura_3_G_diana_3sigma.png', dpi=300)
...
plt.grid(True)
plt.savefig('C:\\Users\\vedq\\Desktop\\desarrollo\\SRC-VED\\Einstein-VED\\img\\EL_figura_4_G_diana_4sigma.png', dpi=300)
...
plt.plot(n_hyper, G_hyper, color='purple', label=r'G(n) ασυμπτωτικό (συνεχές)')
...
plt.legend()
plt.savefig('C:\\Users\\vedq\\Desktop\\desarrollo\\SRC-VED\\Einstein-VED\\img\\EL_figura_5_G_asintotica_log.png', dpi=300)
...
tabla_datos = [
    [r"Concepto", r"Valor [m$^3$/kg$\cdot$s$^2$]", r"Πηγή"],
    [r"$G_{MAX} \pm 3\sigma$", f"{G_max:.8e}", r"Άνω όριο"],
    [r"$G_2^{{EXP}}$ Experimental", f"{G_exp:.8e}", r"Μέση τιμή"], 
    [r"$G_{MIN} \mp 3\sigma$", f"{G_min:.8e}", r"Κάτω όριο"],
    [r"$G_1^{{VED}}$ (n=2) EXACTO", f"{G_n2:.8e}", r"Λήφθηκε εμπειρικά"],
    [r"Diferencia absoluta", f"{error_abs:.8e}", r"$|G_1 - G_2|$"],
    [r"Error relativo", rf"${error_rel:.6f}\,\%$", r"Ακρίβεια VED"]
]
...
plt.title(r"ΒΑΡΥΤΙΚΗ ΣΤΑΘΕΡΑ $G$", fontsize=16, pad=20, weight='bold')
plt.tight_layout()
plt.savefig("C:\\Users\\vedq\\Desktop\\desarrollo\\SRC-VED\\Einstein-VED\\img\\EL_Figura_6_tabla_n2.png", dpi=300, bbox_inches='tight')
...
plt.savefig("C:\\Users\\vedq\\Desktop\\desarrollo\\SRC-VED\\Einstein-VED\\img\\EL_figura_7_materia_holograma.png", dpi=300)
...
plt.savefig('C:\\Users\\vedq\\Desktop\\desarrollo\\SRC-VED\\Einstein-VED\\img\\EL_figura_8_proton_electron.png', dpi=300, bbox_inches='tight')
...
plt.tight_layout()
plt.savefig('C:\\Users\\vedq\\Desktop\\desarrollo\\SRC-VED\\Einstein-VED\\img\\EL_figura_9_tabla_conceptual.png', dpi=300, bbox_inches='tight')
