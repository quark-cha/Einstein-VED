...
def calcular_rango_3sigma(mean, sigma):
    """Berechnet ±3σ-Bereich für Visualisierung"""
...
def crear_grafica_doble_escala_corregida():
    ...
    ax.fill_between([x_positions[0]-width, x_positions[0]+width], mc_min, mc_max,
                    color=c_MC, alpha=0.3, label='MC: Probabilistisch')
    ax.fill_between([x_positions[1]-width, x_positions[1]+width], lhc_min, lhc_max,
                    color=c_LHC, alpha=0.4, label='LHC: Experimentell')
    ax.fill_between([x_positions[2]-width, x_positions[2]+width], ved_min, ved_max,
                    color=c_VED, alpha=0.6, label='Einstein-VED: Deterministisch')
    ...
    ax.set_xticklabels(['Quantenmechanik', 'LHC Experimentell', 'Einstein-VED'], rotation=15)
    ax.set_ylabel('Protonenradius (fm)', fontweight='bold')
    ax.set_title('Protonenradius-Schätzungen\nmit ±3σ-Bereichen', fontweight='bold', fontsize=14)
    ...
    texto_ved = textwrap.fill(
        "Einstein-VED: Deterministisch geometric model of proton. \n" +
        "Radius abgeleitet aus 4 Wellenlängen des Einschlusses.", width=40
    )
    ...
    plt.tight_layout()
    plt.savefig(r"C:\Users\vedq\Desktop\desarrollo\SRC-VED\Einstein-VED\img\SK_proton_doble_escala_corregida.svg", format='svg', dpi=300)
