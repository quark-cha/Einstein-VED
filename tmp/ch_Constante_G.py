...
ax1.plot(theta, r, linestyle='--', color='blue', label='质子半径 (中线)')
# 4 ondas rojas del protón sobre el radio
for k in range(1, n_onda+1):
    ax1.plot(theta, r*(k/n_onda), color='red', linestyle='-', alpha=0.8)
ax1.set_title('图 1: VED约束质子波, n=2', pad=20)
ax1.legend()
plt.savefig(r"C:\Users\vedq\Desktop\desarrollo\SRC-VED\Einstein-VED\img\figura_1_proton.png", dpi=300)
plt.close()

# ===========================
# 4. Figura 3: 黑洞 y superficie 2D
# ===========================
R_s = 2 * G_exp * M_BH / c**2

plt.figure(figsize=(6,6))
ax2 = plt.gca()
circle = plt.Circle((0,0), R_s, color='black', alpha=0.7, label='黑洞')
ax2.add_artist(circle)
circle_surf = plt.Circle((0,0), R_s, color='yellow', fill=False, linewidth=2, label='二维表面')
ax2.add_artist(circle_surf)
ax2.set_xlim(-R_s*1.2, R_s*1.2)
ax2.set_ylim(-R_s*1.2, R_s*1.2)
ax2.set_aspect('equal', 'box')
plt.title('Figura 2: 黑洞 y superficie 2D holográfica', pad=20)
plt.legend()
plt.savefig(r"C:\Users\vedq\Desktop\desarrollo\SRC-VED\Einstein-VED\img\figura_2_proton.png", dpi=300)
plt.close()
...
plt.axhline(G_exp, color='black', linestyle='--', label='实验G值')
...
plt.xlim(0,6)
plt.xlabel('n ∈ Z⁺')
plt.ylabel('计算G [m^3/kg/s^2]')
plt.title('图 3: G靶心图 ±3σ', pad=20)
plt.xticks(np.arange(0,6))
plt.grid(True)
plt.legend()
plt.savefig(r"C:\Users\vedq\Desktop\desarrollo\SRC-VED\Einstein-VED\img\figura_3_G_diana_3sigma.png", dpi=300)
plt.close()
...
plt.xlabel('n ∈ Z⁺')
plt.ylabel('计算G [m^3/kg/s^2]')
plt.title('图 4: G靶心图 ±4σ (箭头)', pad=20)
plt.xticks(np.arange(0,5))
plt.ylim(G_exp - 6*sigma, G_exp + 6*sigma)
plt.grid(True)
plt.legend()
plt.savefig(r"C:\Users\vedq\Desktop\desarrollo\SRC-VED\Einstein-VED\img\figura_4_G_diana_4sigma.png", dpi=300)
plt.close()
...
plt.plot(n_hyper, G_hyper, color='purple', label='G(n)渐近 (连续)')
...
plt.title('图 5: G(n)渐近行为 n→∞ (对数-对数)', pad=20)
plt.grid(True, which='both', linestyle='--', linewidth=0.5)
plt.legend()
plt.savefig(r"C:\Users\vedq\Desktop\desarrollo\SRC-VED\Einstein-VED\img\figura_5_G_asintotica_log.png", dpi=300)
plt.close()
...
# Datos de la tabla
tabla_datos = [
    [" Concepto ", " 数值 [m³/kg·s²] ", " Origen "],
    [r" $G_{MAX}$ +3σ ", f" {G_max:.8e} ", " 上限 "],
    [r" $G_2^{EXP}$ 实验值 ", f" {G_exp:.8e} ", " 平均值 "], 
    [r" $G_{MIN}$ -3σ ", f" {G_min:.8e} ", " 下限 "],
    [r" $G_1^{VED}$ (n=2) 精确值 ", f" {G_n2:.8e} ", " 经验获得 "],
    [" 绝对差异 ", f" {error_abs:.8e} ", " |G₁ - G₂| "],
    [" 相对误差 ", f" {error_rel:.6f} % ", " VED精度 "]
]
...
plt.tight_layout()
plt.savefig(r"C:\Users\vedq\Desktop\desarrollo\SRC-VED\Einstein-VED\img\figura_7_tabla_n2.png", dpi=300, bbox_inches='tight')
plt.close()
...
ax.text(np.pi/2, r[0]*0.7, '约束物质波', color='blue', fontsize=12, 
...
ax.text(5*np.pi/4, L_geom*0.4, '二维全息图', color='orange', fontsize=12, 
...
ax.text(np.pi/2, arrow_r+L_geom*0.35, 'G的涌现', color='green', fontsize=12, ha='center')

ax.set_title('图 8: 物质作为二维全息图上的约束波', fontsize=14, pad=20)
ax.legend(loc='upper right', fontsize=10)
plt.savefig(r"C:\Users\vedq\Desktop\desarrollo\SRC-VED\Einstein-VED\img\figura_8_materia_holograma.png", dpi=300)
plt.close()
...
# Protón
ax1.plot(theta, r_proton, '--', color='blue', linewidth=2, label='质子半径')
ax1.plot(theta, ondas_proton, color='red', linewidth=1.5, label='质子径向波')
ax1.plot(theta, onda2d_proton, color='darkred', linewidth=2, alpha=0.8, label='质子二维波动 (n=4)')

# Etiquetas protón
ax1.text(np.pi/2, r_p * 1.3, '质子半径', color='blue', ha='center', va='center', fontsize=11, weight='bold')
ax1.text(0, r_p * 1.1, '质子波', color='red', ha='center', va='center', fontsize=10)
ax1.text(np.pi, r_p * 1.15, '二维波动', color='darkred', ha='center', va='center', fontsize=9)

ax1.set_title("质子 - 飞米尺度 (10⁻¹⁵ m)", fontsize=12, pad=20)
ax1.set_rticks([])
ax1.grid(False)
...
# Electrón
ax2.plot(theta, r_electron, '--', color='green', linewidth=2, label='电子轨道半径')
ax2.plot(theta, ondas_electron, color='magenta', linewidth=1.3, label='电子径向波')
ax2.plot(theta, onda2d_electron, color='purple', linewidth=1.8, alpha=0.8, label='二维波动 electrón (n=137)')

# Etiquetas electrón
ax2.text(np.pi/2, r_e * 1.1, '电子轨道半径', color='green', ha='center', va='center', fontsize=11, weight='bold')
ax2.text(0, r_e * 0.6, '电子波', color='magenta', ha='center', va='center', fontsize=10)
ax2.text(np.pi, r_e * 0.7, '二维波动', color='purple', ha='center', va='center', fontsize=9)

ax2.set_title("电子 - 原子尺度 (10⁻¹¹ m)", fontsize=12, pad=20)
ax2.set_rticks([])
ax2.grid(False)
...
# Título principal
plt.suptitle("图 9. 质子和电子的二维及径向波\n(全息 → 粒子)", 
             fontsize=16, y=0.95)

plt.tight_layout(pad=3.0)

plt.savefig(r"C:\Users\vedq\Desktop\desarrollo\SRC-VED\Einstein-VED\img\figura_9_proton_electron.png", dpi=300, bbox_inches='tight')
...
tabla_datos = [
    ...
    ['Longitud', r'$L_\mathrm{geom}$ 由' + '\n' + 'VED定义', 
     r'$R_s$ 由定义' + '\n' + 'Schwarzschild', '每边定义\n自己的尺度'],
    ['量子化', 'n整数 → 值' + '\n' + 'M和G的离散值', 
     'n不出现,\nG被确定', 'n=2与\nG的实验值一致'],
    ['Dimensionalidad', '二维约束' + '\n' + '(表面)', 
     '三维 → 全息' + '\n' + '隐含', '引力的涌现起源']
]
...
plt.title('图 10: VED与Schwarzschild概念比较', fontsize=14, pad=25)
plt.tight_layout()
plt.savefig(r"C:\Users\vedq\Desktop\desarrollo\SRC-VED\Einstein-VED\img\figura_10_tabla_conceptual.png", dpi=300, bbox_inches='tight')
plt.close()