...
ax1.plot(theta, r, linestyle='--', color='blue', label=r'Raio do próton (linha média)')
for k in range(1, n_onda+1):
    ax1.plot(theta, r*(k/n_onda), color='red', linestyle='-', alpha=0.8)
ax1.set_title(r'Figura 1: Onda confinada VED do próton, n=2', pad=20)
ax1.legend()
plt.savefig('C:\\Users\\vedq\\Desktop\\desarrollo\\SRC-VED\\Einstein-VED\\img\\PT_figura_1_proton.png', dpi=300)
# plt.show()  # Desactivado en traducciones
plt.close()
print("Figura 1 generada."
      )
# ===========================
# 4. Figura 2: Buraco negro y superficie 2D
# ===========================
R_s = 2 * G_exp * M_BH / c**2

plt.figure(figsize=(6,6))
ax2 = plt.gca()
circle = plt.Circle((0,0), R_s, color='black', alpha=0.7, label=r'Buraco negro')
ax2.add_artist(circle)
circle_surf = plt.Circle((0,0), R_s, color='yellow', fill=False, linewidth=2, label=r'Superfície 2D')
ax2.add_artist(circle_surf)
ax2.set_xlim(-R_s*1.2, R_s*1.2)
ax2.set_ylim(-R_s*1.2, R_s*1.2)
ax2.set_aspect('equal', 'box')
plt.title(r'Figura 2: Buraco negro y superficie 2D holográfica', pad=20)
plt.legend()
plt.savefig('C:\\Users\\vedq\\Desktop\\desarrollo\\SRC-VED\\Einstein-VED\\img\\PT_figura_2_proton.png', dpi=300)
...
plt.ylabel(r'G calculado [m³/kg/s²]')
plt.title(r'Figura 3: Diana de G $\pm 3\sigma$', pad=20)
plt.xticks(np.arange(0,6))
plt.grid(True)
plt.legend()
plt.savefig('C:\\Users\\vedq\\Desktop\\desarrollo\\SRC-VED\\Einstein-VED\\img\\PT_figura_3_G_diana_3sigma.png', dpi=300)
...
plt.grid(True)
plt.savefig('C:\\Users\\vedq\\Desktop\\desarrollo\\SRC-VED\\Einstein-VED\\img\\PT_figura_4_G_diana_4sigma.png', dpi=300)
...
plt.plot(n_hyper, G_hyper, color='purple', label=r'G(n) assintótico (contínuo)')
...
plt.legend()
plt.savefig('C:\\Users\\vedq\\Desktop\\desarrollo\\SRC-VED\\Einstein-VED\\img\\PT_figura_5_G_asintotica_log.png', dpi=300)
...
tabla_datos = [
    [r"Concepto", r"Valor [m$^3$/kg$\cdot$s$^2$]", r"Origem"],
    [r"$G_{MAX} \pm 3\sigma$", f"{G_max:.8e}", r"Limite superior"],
    [r"$G_2^{{EXP}}$ Experimental", f"{G_exp:.8e}", r"Valor médio"], 
    [r"$G_{MIN} \mp 3\sigma$", f"{G_min:.8e}", r"Limite inferior"],
    [r"$G_1^{{VED}}$ (n=2) EXACTO", f"{G_n2:.8e}", r"Obtido empiricamente"],
    [r"Diferencia absoluta", f"{error_abs:.8e}", r"$|G_1 - G_2|$"],
    [r"Error relativo", rf"${error_rel:.6f}\,\%$", r"Precisão VED"]
]
...
plt.title(r"Constante gravitacional $G$", fontsize=16, pad=20, weight='bold')
plt.tight_layout()
plt.savefig("C:\\Users\\vedq\\Desktop\\desarrollo\\SRC-VED\\Einstein-VED\\img\\PT_Figura_6_tabla_n2.png", dpi=300, bbox_inches='tight')
...
plt.savefig("C:\\Users\\vedq\\Desktop\\desarrollo\\SRC-VED\\Einstein-VED\\img\\PT_figura_7_materia_holograma.png", dpi=300)
...
plt.savefig('C:\\Users\\vedq\\Desktop\\desarrollo\\SRC-VED\\Einstein-VED\\img\\PT_figura_8_proton_electron.png', dpi=300, bbox_inches='tight')
...
plt.tight_layout()
plt.savefig('C:\\Users\\vedq\\Desktop\\desarrollo\\SRC-VED\\Einstein-VED\\img\\PT_figura_9_tabla_conceptual.png', dpi=300, bbox_inches='tight')
