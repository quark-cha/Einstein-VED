...
ax1.plot(theta, r, linestyle='--', color='blue', label='Raggio del protone (linea centrale)')
# 4 ondas rojas del protón sobre el radio
for k in range(1, n_onda+1):
    ax1.plot(theta, r*(k/n_onda), color='red', linestyle='-', alpha=0.8)
ax1.set_title('Figura 1: Onda di protone confinata VED, n=2', pad=20)
ax1.legend()
plt.savefig(r"C:\Users\vedq\Desktop\desarrollo\SRC-VED\Einstein-VED\img\IT_figura_1_proton.png", dpi=300)
plt.close()

# ===========================
# 4. Figura 3: Buco nero y superficie 2D
# ===========================
R_s = 2 * G_exp * M_BH / c**2

plt.figure(figsize=(6,6))
ax2 = plt.gca()
circle = plt.Circle((0,0), R_s, color='black', alpha=0.7, label='Buco nero')
ax2.add_artist(circle)
circle_surf = plt.Circle((0,0), R_s, color='yellow', fill=False, linewidth=2, label='Superficie 2D')
ax2.add_artist(circle_surf)
ax2.set_xlim(-R_s*1.2, R_s*1.2)
ax2.set_ylim(-R_s*1.2, R_s*1.2)
ax2.set_aspect('equal', 'box')
plt.title('Figura 2: Buco nero y superficie 2D holográfica', pad=20)
plt.legend()
plt.savefig(r"C:\Users\vedq\Desktop\desarrollo\SRC-VED\Einstein-VED\img\IT_figura_2_proton.png", dpi=300)
plt.close()
...
plt.axhline(G_exp, color='black', linestyle='--', label='G sperimentale')
...
plt.xlim(0,6)
plt.xlabel('n ∈ Z⁺')
plt.ylabel('G calcolato [m^3/kg/s^2]')
plt.title('Figura 3: Bersaglio G ±3σ', pad=20)
plt.xticks(np.arange(0,6))
plt.grid(True)
plt.legend()
plt.savefig(r"C:\Users\vedq\Desktop\desarrollo\SRC-VED\Einstein-VED\img\IT_figura_3_G_diana_3sigma.png", dpi=300)
plt.close()
...
plt.xlabel('n ∈ Z⁺')
plt.ylabel('G calcolato [m^3/kg/s^2]')
plt.title('Figura 4: Bersaglio G ±4σ (frecce)', pad=20)
plt.xticks(np.arange(0,5))
plt.ylim(G_exp - 6*sigma, G_exp + 6*sigma)
plt.grid(True)
plt.legend()
plt.savefig(r"C:\Users\vedq\Desktop\desarrollo\SRC-VED\Einstein-VED\img\IT_figura_4_G_diana_4sigma.png", dpi=300)
plt.close()
...
plt.plot(n_hyper, G_hyper, color='purple', label='G(n) asintotico (continuo)')
...
plt.title('Figura 5: Comportamento asintotico di G(n) n→∞ (log-log)', pad=20)
plt.grid(True, which='both', linestyle='--', linewidth=0.5)
plt.legend()
plt.savefig(r"C:\Users\vedq\Desktop\desarrollo\SRC-VED\Einstein-VED\img\IT_figura_5_G_asintotica_log.png", dpi=300)
plt.close()
...
# Datos de la tabla
tabla_datos = [
    [" Concepto ", " Valore [m³/kg·s²] ", " Origen "],
    [r" $G_{MAX}$ +3σ ", f" {G_max:.8e} ", " Limite superiore "],
    [r" $G_2^{EXP}$ Sperimentale ", f" {G_exp:.8e} ", " Valore medio "], 
    [r" $G_{MIN}$ -3σ ", f" {G_min:.8e} ", " Limite inferiore "],
    [r" $G_1^{VED}$ (n=2) ESATTO ", f" {G_n2:.8e} ", " Ottenuto empiricamente "],
    [" Differenza assoluta ", f" {error_abs:.8e} ", " |G₁ - G₂| "],
    [" Errore relativo ", f" {error_rel:.6f} % ", " Precisione VED "]
]
...
plt.title("COSTANTE GRAVITAZIONALE G", 
          fontsize=16, pad=20, weight='bold')

plt.tight_layout()
plt.savefig(r"C:\Users\vedq\Desktop\desarrollo\SRC-VED\Einstein-VED\img\IT_figura_7_tabla_n2.png", dpi=300, bbox_inches='tight')
plt.close()
...
ax.text(np.pi/2, r[0]*0.7, 'Onda di materia confinata', color='blue', fontsize=12, 
...
ax.text(5*np.pi/4, L_geom*0.4, 'Ologramma 2D', color='orange', fontsize=12, 
...
ax.text(np.pi/2, arrow_r+L_geom*0.35, 'Emergenza di G', color='green', fontsize=12, ha='center')

ax.set_title('Figura 8: Materia come onda confinata su ologramma 2D', fontsize=14, pad=20)
ax.legend(loc='upper right', fontsize=10)
plt.savefig(r"C:\Users\vedq\Desktop\desarrollo\SRC-VED\Einstein-VED\img\IT_figura_8_materia_holograma.png", dpi=300)
plt.close()
...
r_p = 0.84118e-15  # Raggio del protone exacto
...
# Protón
ax1.plot(theta, r_proton, '--', color='blue', linewidth=2, label='Raggio del protone')
ax1.plot(theta, ondas_proton, color='red', linewidth=1.5, label='Onde radiali del protone')
ax1.plot(theta, onda2d_proton, color='darkred', linewidth=2, alpha=0.8, label='Ondulazione 2D del protone (n=4)')

# Etiquetas protón
ax1.text(np.pi/2, r_p * 1.3, 'Raggio del protone', color='blue', ha='center', va='center', fontsize=11, weight='bold')
ax1.text(0, r_p * 1.1, 'Onde del protone', color='red', ha='center', va='center', fontsize=10)
ax1.text(np.pi, r_p * 1.15, 'Ondulazione 2D', color='darkred', ha='center', va='center', fontsize=9)

ax1.set_title("Protone - Scala femtometrica (10⁻¹⁵ m)", fontsize=12, pad=20)
ax1.set_rticks([])
ax1.grid(False)
...
# Electrón
ax2.plot(theta, r_electron, '--', color='green', linewidth=2, label='Raggio orbitale dell'elettrone')
ax2.plot(theta, ondas_electron, color='magenta', linewidth=1.3, label='Onde radiali dell'elettrone')
ax2.plot(theta, onda2d_electron, color='purple', linewidth=1.8, alpha=0.8, label='Ondulazione 2D electrón (n=137)')

# Etiquetas electrón
ax2.text(np.pi/2, r_e * 1.1, 'Raggio orbitale dell'elettrone', color='green', ha='center', va='center', fontsize=11, weight='bold')
ax2.text(0, r_e * 0.6, 'Onde dell'elettrone', color='magenta', ha='center', va='center', fontsize=10)
ax2.text(np.pi, r_e * 0.7, 'Ondulazione 2D', color='purple', ha='center', va='center', fontsize=9)

ax2.set_title("Elettrone - Scala atomica (10⁻¹¹ m)", fontsize=12, pad=20)
ax2.set_rticks([])
ax2.grid(False)
...
# Título principal
plt.suptitle("Figura 9. Onde 2D e radiali del protone e dell'elettrone\n(olografia → particelle)", 
             fontsize=16, y=0.95)

plt.tight_layout(pad=3.0)

plt.savefig(r"C:\Users\vedq\Desktop\desarrollo\SRC-VED\Einstein-VED\img\IT_figura_9_proton_electron.png", dpi=300, bbox_inches='tight')
...
tabla_datos = [
    ...
    ['Masa', r'$M_\mathrm{VED} = n h /$' + '\n' + r'$(L_\mathrm{geom} c)$', 
     r'$M_\mathrm{Sch} = c^2 R_s /$' + '\n' + r'$(2 G)$', 'Uguagliando le masse,\nM scompare'],
    ['Longitud', r'$L_\mathrm{geom}$ definita\nda VED' + '\n' + '$R_s$ definita da', 
     r'Ogni lato definisce\nla propria scala' + '\n' + 'Schwarzschild', 'Quantizzazione'],
    ['n intero → valori\ndiscreti di M e G', 'n non appare,\nG è determinato' + '\n' + 'n=2 coincide con\nvalore sperimentale di G', 
     'Confinamento 2D\nFigura 10: Confronto concettuale VED vs Schwarzschild', '3D → olografia\nimplicita'],
    ['Dimensionalidad', 'Origine emergente\della gravità' + '\n' + 'Figura 10: Confronto concettuale VED vs Schwarzschild', 
...
plt.tight_layout()
plt.savefig(r"C:\Users\vedq\Desktop\desarrollo\SRC-VED\Einstein-VED\img\IT_figura_10_tabla_conceptual.png", dpi=300, bbox_inches='tight')
plt.close()