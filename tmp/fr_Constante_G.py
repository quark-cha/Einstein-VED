...
ax1.plot(theta, r, linestyle='--', color='blue', label='Rayon du proton (ligne médiane)')
# 4 ondas rojas del protón sobre el radio
for k in range(1, n_onda+1):
    ax1.plot(theta, r*(k/n_onda), color='red', linestyle='-', alpha=0.8)
ax1.set_title('Figure 1 : Onde de proton confinée VED, n=2', pad=20)
ax1.legend()
plt.savefig(r"C:\Users\vedq\Desktop\desarrollo\SRC-VED\Einstein-VED\img\FR_figura_1_proton.png", dpi=300)
plt.close()

# ===========================
# 4. Figura 3: Trou noir y superficie 2D
# ===========================
R_s = 2 * G_exp * M_BH / c**2

plt.figure(figsize=(6,6))
ax2 = plt.gca()
circle = plt.Circle((0,0), R_s, color='black', alpha=0.7, label='Trou noir')
ax2.add_artist(circle)
circle_surf = plt.Circle((0,0), R_s, color='yellow', fill=False, linewidth=2, label='Surface 2D')
ax2.add_artist(circle_surf)
ax2.set_xlim(-R_s*1.2, R_s*1.2)
ax2.set_ylim(-R_s*1.2, R_s*1.2)
ax2.set_aspect('equal', 'box')
plt.title('Figura 2: Trou noir y superficie 2D holográfica', pad=20)
plt.legend()
plt.savefig(r"C:\Users\vedq\Desktop\desarrollo\SRC-VED\Einstein-VED\img\FR_figura_2_proton.png", dpi=300)
plt.close()
...
plt.axhline(G_exp, color='black', linestyle='--', label='G expérimental')
...
plt.xlim(0,6)
plt.xlabel('n ∈ Z⁺')
plt.ylabel('G calculé [m^3/kg/s^2]')
plt.title('Figure 3 : Cible G ±3σ', pad=20)
plt.xticks(np.arange(0,6))
plt.grid(True)
plt.legend()
plt.savefig(r"C:\Users\vedq\Desktop\desarrollo\SRC-VED\Einstein-VED\img\FR_figura_3_G_diana_3sigma.png", dpi=300)
plt.close()
...
plt.xlabel('n ∈ Z⁺')
plt.ylabel('G calculé [m^3/kg/s^2]')
plt.title('Figure 4 : Cible G ±4σ (flèches)', pad=20)
plt.xticks(np.arange(0,5))
plt.ylim(G_exp - 6*sigma, G_exp + 6*sigma)
plt.grid(True)
plt.legend()
plt.savefig(r"C:\Users\vedq\Desktop\desarrollo\SRC-VED\Einstein-VED\img\FR_figura_4_G_diana_4sigma.png", dpi=300)
plt.close()
...
plt.plot(n_hyper, G_hyper, color='purple', label='G(n) asymptotique (continu)')
...
plt.title('Figure 5 : Comportement asymptotique de G(n) n→∞ (log-log)', pad=20)
plt.grid(True, which='both', linestyle='--', linewidth=0.5)
plt.legend()
plt.savefig(r"C:\Users\vedq\Desktop\desarrollo\SRC-VED\Einstein-VED\img\FR_figura_5_G_asintotica_log.png", dpi=300)
plt.close()
...
# Datos de la tabla
tabla_datos = [
    [" Concepto ", " Valeur [m³/kg·s²] ", " Origen "],
    [r" $G_{MAX}$ +3σ ", f" {G_max:.8e} ", " Limite supérieure "],
    [r" $G_2^{EXP}$ Expérimental ", f" {G_exp:.8e} ", " Valeur moyenne "], 
    [r" $G_{MIN}$ -3σ ", f" {G_min:.8e} ", " Limite inférieure "],
    [r" $G_1^{VED}$ (n=2) EXACT ", f" {G_n2:.8e} ", " Obtenu empiriquement "],
    [" Différence absolue ", f" {error_abs:.8e} ", " |G₁ - G₂| "],
    [" Erreur relative ", f" {error_rel:.6f} % ", " Précision VED "]
]
...
plt.title("CONSTANTE GRAVITATIONNELLE G", 
          fontsize=16, pad=20, weight='bold')

plt.tight_layout()
plt.savefig(r"C:\Users\vedq\Desktop\desarrollo\SRC-VED\Einstein-VED\img\FR_figura_7_tabla_n2.png", dpi=300, bbox_inches='tight')
plt.close()
...
ax.text(np.pi/2, r[0]*0.7, 'Onde de matière confinée', color='blue', fontsize=12, 
...
ax.text(5*np.pi/4, L_geom*0.4, 'Hologramme 2D', color='orange', fontsize=12, 
...
ax.text(np.pi/2, arrow_r+L_geom*0.35, 'Émergence de G', color='green', fontsize=12, ha='center')

ax.set_title('Figure 8 : Matière comme onde confinée sur hologramme 2D', fontsize=14, pad=20)
ax.legend(loc='upper right', fontsize=10)
plt.savefig(r"C:\Users\vedq\Desktop\desarrollo\SRC-VED\Einstein-VED\img\FR_figura_8_materia_holograma.png", dpi=300)
plt.close()
...
r_p = 0.84118e-15  # Rayon du proton exacto
...
# Protón
ax1.plot(theta, r_proton, '--', color='blue', linewidth=2, label='Rayon du proton')
ax1.plot(theta, ondas_proton, color='red', linewidth=1.5, label='Ondes radiales du proton')
ax1.plot(theta, onda2d_proton, color='darkred', linewidth=2, alpha=0.8, label='Ondulation 2D du proton (n=4)')

# Etiquetas protón
ax1.text(np.pi/2, r_p * 1.3, 'Rayon du proton', color='blue', ha='center', va='center', fontsize=11, weight='bold')
ax1.text(0, r_p * 1.1, 'Ondes du proton', color='red', ha='center', va='center', fontsize=10)
ax1.text(np.pi, r_p * 1.15, 'Ondulation 2D', color='darkred', ha='center', va='center', fontsize=9)

ax1.set_title("Proton - Échelle femtométrique (10⁻¹⁵ m)", fontsize=12, pad=20)
ax1.set_rticks([])
ax1.grid(False)
...
# Electrón
ax2.plot(theta, r_electron, '--', color='green', linewidth=2, label='Rayon orbital de l'électron')
ax2.plot(theta, ondas_electron, color='magenta', linewidth=1.3, label='Ondes radiales de l'électron')
ax2.plot(theta, onda2d_electron, color='purple', linewidth=1.8, alpha=0.8, label='Ondulation 2D electrón (n=137)')

# Etiquetas electrón
ax2.text(np.pi/2, r_e * 1.1, 'Rayon orbital de l'électron', color='green', ha='center', va='center', fontsize=11, weight='bold')
ax2.text(0, r_e * 0.6, 'Ondes de l'électron', color='magenta', ha='center', va='center', fontsize=10)
ax2.text(np.pi, r_e * 0.7, 'Ondulation 2D', color='purple', ha='center', va='center', fontsize=9)

ax2.set_title("Électron - Échelle atomique (10⁻¹¹ m)", fontsize=12, pad=20)
ax2.set_rticks([])
ax2.grid(False)
...
# Título principal
plt.suptitle("Figure 9. Ondes 2D et radiales du proton et de l'électron\n(holographie → particules)", 
             fontsize=16, y=0.95)

plt.tight_layout(pad=3.0)

plt.savefig(r"C:\Users\vedq\Desktop\desarrollo\SRC-VED\Einstein-VED\img\FR_figura_9_proton_electron.png", dpi=300, bbox_inches='tight')
...
tabla_datos = [
    ...
    ['Masa', r'$M_\mathrm{VED} = n h /$' + '\n' + r'$(L_\mathrm{geom} c)$', 
     r'$M_\mathrm{Sch} = c^2 R_s /$' + '\n' + r'$(2 G)$', 'Égalisation des masses,\nM disparaît'],
    ['Longitud', r'$L_\mathrm{geom}$ définie\npar VED' + '\n' + '$R_s$ définie par', 
     r'Chaque côté définit\nsa propre échelle' + '\n' + 'Schwarzschild', 'Quantification'],
    ['n entier → valeurs\ndiscrètes de M et G', 'n n'apparaît pas,\nG est déterminé' + '\n' + 'n=2 coïncide avec\nvaleur expérimentale de G', 
     'Confinement 2D\n(surface)', '3D → holographie\nimplicite'],
    ['Dimensionalidad', 'Origine émergente\nde la gravité' + '\n' + 'Figure 10 : Comparaison conceptuelle VED vs Schwarzschild', 
...
plt.tight_layout()
plt.savefig(r"C:\Users\vedq\Desktop\desarrollo\SRC-VED\Einstein-VED\img\FR_figura_10_tabla_conceptual.png", dpi=300, bbox_inches='tight')
plt.close()