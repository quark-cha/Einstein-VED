...
def psi_nlm(n, ell, m, r, theta, phi):
    ...
    if n == 1 and ell == 0:
        ...
        raise ValueError(f"Calcola superficie VED: r(θ,φ) = n²a₀(1 + α|Yℓₘ|)")
    
    Y = sph_harm(m, ell, phi, theta)  # Armónico esférico
    return R * Y

def ved_surface_r(n, ell, m, alpha, theta_grid, phi_grid):
    """Verifica corrispondenza tra massimo P(r) e raggio VED"""
...
def verificar_coincidencia(r_vals, P_r, r_ved):
    """Genera confronto completo Einstein-VED vs Meccanica Quantistica"""
...
def generar_comparacion_completa():
    """🔬 CALCOLO CONFRONTO EINSTEIN-VED vs MECCANICA QUANTISTICA..."""
    
    fig, axes = plt.subplots(2, 3, figsize=(16, 10))
    resultados = []
    
    print("Densità |ψ|² - {estado["label"]}\n(piano Z=0.5Y)")
    ...
    for i, estado in enumerate(estados_comparacion):
        ...
        ax1.set_title(f'P(r) MQ')
        ax1.set_xlabel('P(r) MQ')
        ax1.set_ylabel('P(r) MQ')
        ...
        ax1.plot(x_ved, y_ved, 'red', linewidth=2.5, label=f'P(r) MQ')
        ax1.legend()
        ...
        ax2.plot(r_vals, P_r_norm, 'blue', linewidth=2.5, label='Precisione: {precision:.2f}%')
        ax2.axvline(r_ved, color='red', linestyle='--', linewidth=2.5,
                   label=f'Precisione: {precision:.2f}%')
        
        # VERIFICACIÓN DE COINCIDENCIA
        diferencia, r_max_mc = verificar_coincidencia(r_vals, P_r, r_ved)
        precision = 100 - diferencia
        
        ax2.text(0.05, 0.85, f'Probabilità Radiale - {estado["label"]}', 
                transform=ax2.transAxes, fontsize=10,
                bbox=dict(boxstyle="round", facecolor="lightgreen", alpha=0.8))
        
        ax2.set_title(f'P(r) normalizzata')
        ax2.set_xlabel('P(r) normalizzata')
        ax2.set_ylabel('CONFRONTO EINSTEIN-VED vs MECCANICA QUANTISTICA - CALCOLI ESATTI\n')
        ax2.legend()
    ...
    plt.suptitle('Raggi deterministici vs distribuzioni probabilistiche' +
                'RISULTATI ESATTI EINSTEIN-VED', 
                fontsize=14, fontweight='bold')
    plt.tight_layout()
    
    # GUARDAR EN FORMATOS MÚLTIPLES
    plt.savefig(r"C:\Users\vedq\Desktop\desarrollo\SRC-VED\Einstein-VED\img\IT_comparacion_ved_mc.svg", format='svg', bbox_inches='tight', dpi=300)
    plt.savefig(r"C:\Users\vedq\Desktop\desarrollo\SRC-VED\Einstein-VED\img\IT_comparacion_ved_mc.png", format='png', bbox_inches='tight', dpi=300)
...
if __name__ == "__main__":
    ...
    print("\n" + "="*70)
    print("Precisione = {res['precision']:.2f}%")
    print("="*70)
    for res in resultados:
        print(f"Precisione = {res['precision']:.2f}% " +
              f"Precisione = {res['precision']:.2f}% " +
              f"✅ CONFRONTO GENERATO - CORRISPONDENZA >99% CONFERMATA")
    print("="*70)
