...
def crear_animacion_confinamiento():
    ...
    ax.set_ylabel('Ampiezza ψ(x)', fontsize=12)
    ax.set_title('CONFINAMENTO EINSTEIN-VED: Onda → Particella', fontweight='bold', fontsize=14)
    ...
    def animate(frame):
        ...
        if frame < 30:  # FASE 1: Onda completa en papel
            ...
            ax.set_xlabel('Posizione x', fontsize=12)
            ax.set_ylabel('Ampiezza ψ(x)', fontsize=12)
            ax.set_title('1. Onda Sinusoidale su Superficie Piana', fontweight='bold')
            ...
            ax.set_title(f'2. Transizione: Avvolgimento ({progress*100:.0f}%)', fontweight='bold')
            ...
            ax.plot(x_wave, y_wave, 'red', linewidth=3, label=f'Confinata (n={n_cycles})')
            ax.text(0.4, -1.8, f'CONDIZIONE EINSTEIN-VED: L = {n_cycles}λ', 
                    fontweight='bold', fontsize=12,
                    bbox=dict(boxstyle="round", facecolor="yellow"))
            ax.set_title('3. Onda Auto-Confinata - Particella Stabile', fontweight='bold')
            ax.set_xlabel('Coordinata X')
            ax.set_ylabel('Coordinata Y')
            ax.legend()
...
def generar_resumen_confinamiento():
    ...
    ax1.set_title('a) Onda Piana Libera', fontweight='bold')
    ax1.set_xlabel('Posizione x')
    ax1.set_ylabel('Ampiezza ψ(x)')
    ...
    ax2.set_title('b) Transizione', fontweight='bold')
    ...
    ax3.set_title('c) Confinata n=3\nL = 3λ', fontweight='bold')
    ax3.set_xlabel('Coordinata X')
    ax3.set_ylabel('Coordinata Y')
    ax3.grid(True, alpha=0.3)
    ax3.text(1.6, -1.3, 'CONDIZIONE: L = nλ', ha='center', fontweight='bold',
             bbox=dict(boxstyle="round", facecolor="yellow"))

    plt.tight_layout()
    plt.savefig(r"C:\Users\vedq\Desktop\desarrollo\SRC-VED\Einstein-VED\img\IT_confinamiento.svg", format='svg', bbox_inches='tight')
    print("SVG resumen guardado: 'resumen_confinamiento.svg'")

# Ejecutar
print("Generazione animazione SVG del confinamento...")
crear_animacion_confinamiento()
generar_resumen_confinamiento()
print("✅ Animazione SVG completata")
