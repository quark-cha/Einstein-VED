...
def calcular_rango_3sigma(mean, sigma):
    """Vypočíta rozsah ±3σ pre vizualizáciu"""
...
def crear_grafica_doble_escala_corregida():
    ...
    ax.fill_between([x_positions[0]-width, x_positions[0]+width], mc_min, mc_max,
                    color=c_MC, alpha=0.3, label='MC: Pravdepodobnostný')
    ax.fill_between([x_positions[1]-width, x_positions[1]+width], lhc_min, lhc_max,
                    color=c_LHC, alpha=0.4, label='LHC: Experimentálny')
    ax.fill_between([x_positions[2]-width, x_positions[2]+width], ved_min, ved_max,
                    color=c_VED, alpha=0.6, label='Einstein-VED: Deterministický')
    ...
    ax.set_xticklabels(['Kvantová mechanika', 'LHC Experimentálny', 'Einstein-VED'], rotation=15)
    ax.set_ylabel('Polomer protónu (fm)', fontweight='bold')
    ax.set_title('Odhad polomeru protónu\ns rozsahmi ±3σ', fontweight='bold', fontsize=14)
    ...
    texto_ved = textwrap.fill(
        "Einstein-VED: Deterministický geometric model of proton. \n" +
        "Polomer odvodený z 4 vlnových dĺžok uväznenia.", width=40
    )
    ...
    plt.tight_layout()
    plt.savefig(r"C:\Users\vedq\Desktop\desarrollo\SRC-VED\Einstein-VED\img\IT_proton_doble_escala_corregida.svg", format='svg', dpi=300)
