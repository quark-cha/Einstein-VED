...
ax1.plot(theta, r, linestyle='--', color='blue', label='Protonenradius (Mittellinie)')
# 4 ondas rojas del protón sobre el radio
for k in range(1, n_onda+1):
    ax1.plot(theta, r*(k/n_onda), color='red', linestyle='-', alpha=0.8)
ax1.set_title('Abbildung 1: VED eingeschlossene Protonwelle, n=2', pad=20)
ax1.legend()
plt.savefig(r"C:\Users\vedq\Desktop\desarrollo\SRC-VED\Einstein-VED\img\DE_figura_1_proton.png", dpi=300)
plt.close()

# ===========================
# 4. Figura 3: Schwarzes Loch y superficie 2D
# ===========================
R_s = 2 * G_exp * M_BH / c**2

plt.figure(figsize=(6,6))
ax2 = plt.gca()
circle = plt.Circle((0,0), R_s, color='black', alpha=0.7, label='Schwarzes Loch')
ax2.add_artist(circle)
circle_surf = plt.Circle((0,0), R_s, color='yellow', fill=False, linewidth=2, label='2D-Oberfläche')
ax2.add_artist(circle_surf)
ax2.set_xlim(-R_s*1.2, R_s*1.2)
ax2.set_ylim(-R_s*1.2, R_s*1.2)
ax2.set_aspect('equal', 'box')
plt.title('Figura 2: Schwarzes Loch y superficie 2D holográfica', pad=20)
plt.legend()
plt.savefig(r"C:\Users\vedq\Desktop\desarrollo\SRC-VED\Einstein-VED\img\DE_figura_2_proton.png", dpi=300)
plt.close()
...
plt.axhline(G_exp, color='black', linestyle='--', label='G experimentell')
...
plt.xlim(0,6)
plt.xlabel('n ∈ Z⁺')
plt.ylabel('G berechnet [m^3/kg/s^2]')
plt.title('Abbildung 3: G Zielscheibe ±3σ', pad=20)
plt.xticks(np.arange(0,6))
plt.grid(True)
plt.legend()
plt.savefig(r"C:\Users\vedq\Desktop\desarrollo\SRC-VED\Einstein-VED\img\DE_figura_3_G_diana_3sigma.png", dpi=300)
plt.close()
...
plt.xlabel('n ∈ Z⁺')
plt.ylabel('G berechnet [m^3/kg/s^2]')
plt.title('Abbildung 4: G Zielscheibe ±4σ (Pfeile)', pad=20)
plt.xticks(np.arange(0,5))
plt.ylim(G_exp - 6*sigma, G_exp + 6*sigma)
plt.grid(True)
plt.legend()
plt.savefig(r"C:\Users\vedq\Desktop\desarrollo\SRC-VED\Einstein-VED\img\DE_figura_4_G_diana_4sigma.png", dpi=300)
plt.close()
...
plt.plot(n_hyper, G_hyper, color='purple', label='G(n) asymptotisch (kontinuierlich)')
...
plt.title('Abbildung 5: Asymptotisches Verhalten von G(n) n→∞ (log-log)', pad=20)
plt.grid(True, which='both', linestyle='--', linewidth=0.5)
plt.legend()
plt.savefig(r"C:\Users\vedq\Desktop\desarrollo\SRC-VED\Einstein-VED\img\DE_figura_5_G_asintotica_log.png", dpi=300)
plt.close()
...
# Datos de la tabla
tabla_datos = [
    [" Concepto ", " Wert [m³/kg·s²] ", " Origen "],
    [r" $G_{MAX}$ +3σ ", f" {G_max:.8e} ", " Obergrenze "],
    [r" $G_2^{EXP}$ Experimentell ", f" {G_exp:.8e} ", " Mittelwert "], 
    [r" $G_{MIN}$ -3σ ", f" {G_min:.8e} ", " Untergrenze "],
    [r" $G_1^{VED}$ (n=2) EXAKT ", f" {G_n2:.8e} ", " Empirisch erhalten "],
    [" Absolute Differenz ", f" {error_abs:.8e} ", " |G₁ - G₂| "],
    [" Relativer Fehler ", f" {error_rel:.6f} % ", " VED Genauigkeit "]
]
...
plt.title("GRAVITATIONSKONSTANTE G", 
          fontsize=16, pad=20, weight='bold')

plt.tight_layout()
plt.savefig(r"C:\Users\vedq\Desktop\desarrollo\SRC-VED\Einstein-VED\img\DE_figura_7_tabla_n2.png", dpi=300, bbox_inches='tight')
plt.close()
...
ax.text(np.pi/2, r[0]*0.7, 'Eingeschlossene Materiewelle', color='blue', fontsize=12, 
...
ax.text(5*np.pi/4, L_geom*0.4, '2D-Hologramm', color='orange', fontsize=12, 
...
ax.text(np.pi/2, arrow_r+L_geom*0.35, 'Emergenz von G', color='green', fontsize=12, ha='center')

ax.set_title('Abbildung 8: Materie als eingeschlossene Welle auf 2D-Hologramm', fontsize=14, pad=20)
ax.legend(loc='upper right', fontsize=10)
plt.savefig(r"C:\Users\vedq\Desktop\desarrollo\SRC-VED\Einstein-VED\img\DE_figura_8_materia_holograma.png", dpi=300)
plt.close()
...
r_p = 0.84118e-15  # Protonenradius exacto
...
# Protón
ax1.plot(theta, r_proton, '--', color='blue', linewidth=2, label='Protonenradius')
ax1.plot(theta, ondas_proton, color='red', linewidth=1.5, label='Radiale Protonwellen')
ax1.plot(theta, onda2d_proton, color='darkred', linewidth=2, alpha=0.8, label='2D-Protonwelligkeit (n=4)')

# Etiquetas protón
ax1.text(np.pi/2, r_p * 1.3, 'Protonenradius', color='blue', ha='center', va='center', fontsize=11, weight='bold')
ax1.text(0, r_p * 1.1, 'Protonwellen', color='red', ha='center', va='center', fontsize=10)
ax1.text(np.pi, r_p * 1.15, '2D-Welligkeit', color='darkred', ha='center', va='center', fontsize=9)

ax1.set_title("Proton - Femtometerskala (10⁻¹⁵ m)", fontsize=12, pad=20)
ax1.set_rticks([])
ax1.grid(False)
...
# Electrón
ax2.plot(theta, r_electron, '--', color='green', linewidth=2, label='Elektronenorbitalradius')
ax2.plot(theta, ondas_electron, color='magenta', linewidth=1.3, label='Radiale Elektronwellen')
ax2.plot(theta, onda2d_electron, color='purple', linewidth=1.8, alpha=0.8, label='2D-Welligkeit electrón (n=137)')

# Etiquetas electrón
ax2.text(np.pi/2, r_e * 1.1, 'Elektronenorbitalradius', color='green', ha='center', va='center', fontsize=11, weight='bold')
ax2.text(0, r_e * 0.6, 'Elektronwellen', color='magenta', ha='center', va='center', fontsize=10)
ax2.text(np.pi, r_e * 0.7, '2D-Welligkeit', color='purple', ha='center', va='center', fontsize=9)

ax2.set_title("Elektron - Atomskala (10⁻¹¹ m)", fontsize=12, pad=20)
ax2.set_rticks([])
ax2.grid(False)
...
# Título principal
plt.suptitle("Abbildung 9. 2D- und radiale Wellen von Proton und Elektron\n(Holographie → Teilchen)", 
             fontsize=16, y=0.95)

plt.tight_layout(pad=3.0)

plt.savefig(r"C:\Users\vedq\Desktop\desarrollo\SRC-VED\Einstein-VED\img\DE_figura_9_proton_electron.png", dpi=300, bbox_inches='tight')
...
tabla_datos = [
    ...
    ['Masa', r'$M_\mathrm{VED} = n h /$' + '\n' + r'$(L_\mathrm{geom} c)$', 
     r'$M_\mathrm{Sch} = c^2 R_s /$' + '\n' + r'$(2 G)$', 'Massen gleichsetzen,\nM verschwindet'],
    ['Longitud', r'$L_\mathrm{geom}$ definiert' + '\n' + 'durch VED', 
     r'$R_s$ definiert durch' + '\n' + 'Schwarzschild', 'Jede Seite definiert\neigene Skala'],
    ['Quantisierung', 'n ganzzahlig → diskrete' + '\n' + 'Werte von M und G', 
     'n erscheint nicht,\nG wird bestimmt', 'n=2 stimmt mit\nexperimentellem G-Wert überein'],
    ['Dimensionalidad', '2D-Confinement' + '\n' + '(Oberfläche)', 
     '3D → implizite' + '\n' + 'Holographie', 'Emergenter Ursprung\nder Gravitation']
]
...
plt.title('Abbildung 10: Konzeptvergleich VED vs Schwarzschild', fontsize=14, pad=25)
plt.tight_layout()
plt.savefig(r"C:\Users\vedq\Desktop\desarrollo\SRC-VED\Einstein-VED\img\DE_figura_10_tabla_conceptual.png", dpi=300, bbox_inches='tight')
plt.close()