...
def crear_animacion_confinamiento():
    ...
    line, = ax.plot([], [], 'b-', linewidth=3, label='Wave ψ(x)')
    
    # Ejes y etiquetas iniciales
    ax.set_xlabel('Position x', fontsize=12)
    ax.set_ylabel('Amplitude ψ(x)', fontsize=12)
    ax.set_title('EINSTEIN-VED CONFINEMENT: Wave → Particle', fontweight='bold', fontsize=14)
    ...
    def animate(frame):
        ...
        if frame < 30:  # FASE 1: Onda completa en papel
            ...
            ax.set_xlabel('Position x', fontsize=12)
            ax.set_ylabel('Amplitude ψ(x)', fontsize=12)
            ax.set_title('1. Sinusoidal Wave on Flat Surface', fontweight='bold')
            ...
            ax.set_title(f'2. Transition: Coiling ({progress*100:.0f}%)', fontweight='bold')
            ...
            ax.plot(x_wave, y_wave, 'red', linewidth=3, label=f'Confined (n={n_cycles})')
            ax.text(0.4, -1.8, f'EINSTEIN-VED CONDITION: L = {n_cycles}λ', 
                    fontweight='bold', fontsize=12,
                    bbox=dict(boxstyle="round", facecolor="yellow"))
            ax.set_title('3. Self-Confined Wave - Stable Particle', fontweight='bold')
            ax.set_xlabel('X Coordinate')
            ax.set_ylabel('Y Coordinate')
            ax.legend()
...
def generar_resumen_confinamiento():
    ...
    ax1.set_title('a) Free Plane Wave', fontweight='bold')
    ...
    plt.tight_layout()
    plt.savefig(r"C:\Users\vedq\Desktop\desarrollo\SRC-VED\Einstein-VED\img\EN_confinamiento.svg", format='svg', bbox_inches='tight')
    print("SVG resumen guardado: 'resumen_confinamiento.svg'")

# Ejecutar
print("Generating SVG animation of confinement...")
crear_animacion_confinamiento()
generar_resumen_confinamiento()
print("✅ SVG animation completed")
