...
ax1.plot(theta, r, linestyle='--', color='blue', label=r'نصف قطر البروتون (الخط المتوسط)')
for k in range(1, n_onda+1):
    ax1.plot(theta, r*(k/n_onda), color='red', linestyle='-', alpha=0.8)
ax1.set_title(r'الشكل 1: الموجة المحصورة VED للبروتون، n=2', pad=20)
ax1.legend()
plt.savefig('C:\\Users\\vedq\\Desktop\\desarrollo\\SRC-VED\\Einstein-VED\\img\\AR_figura_1_proton.png', dpi=300)
# plt.show()  # Desactivado en traducciones
plt.close()
print("Figura 1 generada."
      )
# ===========================
# 4. Figura 2: الثقب الأسود y superficie 2D
# ===========================
R_s = 2 * G_exp * M_BH / c**2

plt.figure(figsize=(6,6))
ax2 = plt.gca()
circle = plt.Circle((0,0), R_s, color='black', alpha=0.7, label=r'الثقب الأسود')
ax2.add_artist(circle)
circle_surf = plt.Circle((0,0), R_s, color='yellow', fill=False, linewidth=2, label=r'السطح ثنائي الأبعاد')
ax2.add_artist(circle_surf)
ax2.set_xlim(-R_s*1.2, R_s*1.2)
ax2.set_ylim(-R_s*1.2, R_s*1.2)
ax2.set_aspect('equal', 'box')
plt.title(r'Figura 2: الثقب الأسود y superficie 2D holográfica', pad=20)
plt.legend()
plt.savefig('C:\\Users\\vedq\\Desktop\\desarrollo\\SRC-VED\\Einstein-VED\\img\\AR_figura_2_proton.png', dpi=300)
...
plt.axhline(G_exp, color='black', linestyle='--', label=r'G التجريبي')
...
plt.ylabel(r'G محسوبة [م³/كغ/ث²]')
plt.title(r'Figura 3: Diana de G $\pm 3\sigma$', pad=20)
plt.xticks(np.arange(0,6))
plt.grid(True)
plt.legend()
plt.savefig('C:\\Users\\vedq\\Desktop\\desarrollo\\SRC-VED\\Einstein-VED\\img\\AR_figura_3_G_diana_3sigma.png', dpi=300)
...
plt.grid(True)
plt.savefig('C:\\Users\\vedq\\Desktop\\desarrollo\\SRC-VED\\Einstein-VED\\img\\AR_figura_4_G_diana_4sigma.png', dpi=300)
...
plt.plot(n_hyper, G_hyper, color='purple', label=r'G(n) متقاربة (مستمرة)')
...
plt.xlabel('n')
plt.ylabel(r'G(n) [م³/كغ/ث²]')
plt.title(r'Figura 5: Comportamiento asintótico de $G(n)$ $n \to \infty$ (log-log)', pad=20)
plt.grid(True, which='both', linestyle='--', linewidth=0.5)
plt.legend()
plt.savefig('C:\\Users\\vedq\\Desktop\\desarrollo\\SRC-VED\\Einstein-VED\\img\\AR_figura_5_G_asintotica_log.png', dpi=300)
...
tabla_datos = [
    [r"Concepto", r"Valor [m$^3$/kg$\cdot$s$^2$]", r"المصدر"],
    [r"$G_{MAX} \pm 3\sigma$", f"{G_max:.8e}", r"الحد الأعلى"],
    [r"$G_2^{{EXP}}$ Experimental", f"{G_exp:.8e}", r"القيمة المتوسطة"], 
    [r"$G_{MIN} \mp 3\sigma$", f"{G_min:.8e}", r"الحد الأدنى"],
    [r"$G_1^{{VED}}$ (n=2) EXACTO", f"{G_n2:.8e}", r"تم الحصول عليه تجريبياً"],
    [r"Diferencia absoluta", f"{error_abs:.8e}", r"|G1 - G2|"],
    [r"Error relativo", rf"${error_rel:.6f}\,\%$", r"دقة VED"]
]
...
plt.title(r"الثابت الجاذبي G", fontsize=16, pad=20, weight='bold')
plt.tight_layout()
plt.savefig("C:\\Users\\vedq\\Desktop\\desarrollo\\SRC-VED\\Einstein-VED\\img\\AR_Figura_6_tabla_n2.png", dpi=300, bbox_inches='tight')
...
plt.savefig("C:\\Users\\vedq\\Desktop\\desarrollo\\SRC-VED\\Einstein-VED\\img\\AR_figura_7_materia_holograma.png", dpi=300)
...
plt.savefig('C:\\Users\\vedq\\Desktop\\desarrollo\\SRC-VED\\Einstein-VED\\img\\AR_figura_8_proton_electron.png', dpi=300, bbox_inches='tight')
...
plt.tight_layout()
plt.savefig('C:\\Users\\vedq\\Desktop\\desarrollo\\SRC-VED\\Einstein-VED\\img\\AR_figura_9_tabla_conceptual.png', dpi=300, bbox_inches='tight')
