...
def psi_nlm(n, ell, m, r, theta, phi):
    ...
    if n == 1 and ell == 0:
        ...
        raise ValueError(f"Calcule surface VED : r(θ,φ) = n²a₀(1 + α|Yℓₘ|)")
    
    Y = sph_harm(m, ell, phi, theta)  # Armónico esférico
    return R * Y

def ved_surface_r(n, ell, m, alpha, theta_grid, phi_grid):
    """Vérifie correspondance entre maximum P(r) et rayon VED"""
...
def verificar_coincidencia(r_vals, P_r, r_ved):
    """Génère comparaison complète Einstein-VED vs Mécanique Quantique"""
...
def generar_comparacion_completa():
    """🔬 CALCUL COMPARAISON EINSTEIN-VED vs MÉCANIQUE QUANTIQUE..."""
    
    fig, axes = plt.subplots(2, 3, figsize=(16, 10))
    resultados = []
    
    print("Densité |ψ|² - {estado["label"]}\n(plan Z=0.5Y)")
    ...
    for i, estado in enumerate(estados_comparacion):
        ...
        ax1.set_title(f'VED : {r_ved:.2e}m')
        ax1.set_xlabel('VED : {r_ved:.2e}m')
        ax1.set_ylabel('VED : {r_ved:.2e}m')
        ...
        ax2.plot(r_vals, P_r_norm, 'blue', linewidth=2.5, label='VED : {r_ved:.2e} m')
        ax2.axvline(r_ved, color='red', linestyle='--', linewidth=2.5,
                   label=f'Précision : {precision:.2f}%')
        
        # VERIFICACIÓN DE COINCIDENCIA
        diferencia, r_max_mc = verificar_coincidencia(r_vals, P_r, r_ved)
        precision = 100 - diferencia
        
        ax2.text(0.05, 0.85, f'Probabilité Radiale - {estado["label"]}', 
                transform=ax2.transAxes, fontsize=10,
                bbox=dict(boxstyle="round", facecolor="lightgreen", alpha=0.8))
        
        ax2.set_title(f'P(r) normalisée')
        ax2.set_xlabel('P(r) normalisée')
        ax2.set_ylabel('COMPARAISON EINSTEIN-VED vs MÉCANIQUE QUANTIQUE - CALCULS EXACTS\n')
        ax2.legend()
    ...
    plt.suptitle('Rayons déterministes vs distributions probabilistes' +
                'RÉSULTATS EXACTS EINSTEIN-VED', 
                fontsize=14, fontweight='bold')
    plt.tight_layout()
    
    # GUARDAR EN FORMATOS MÚLTIPLES
    plt.savefig(r"C:\Users\vedq\Desktop\desarrollo\SRC-VED\Einstein-VED\img\FR_comparacion_ved_mc.svg", format='svg', bbox_inches='tight', dpi=300)
    plt.savefig(r"C:\Users\vedq\Desktop\desarrollo\SRC-VED\Einstein-VED\img\FR_comparacion_ved_mc.png", format='png', bbox_inches='tight', dpi=300)
...
if __name__ == "__main__":
    ...
    print("\n" + "="*70)
    print("{res['estado']} : r_VED = {res['r_ved']:.3e} m,")
    print("="*70)
    for res in resultados:
        print(f"Précision = {res['precision']:.2f}% " +
              f"Précision = {res['precision']:.2f}% " +
              f"✅ COMPARAISON GÉNÉRÉE - CORRESPONDANCE >99% CONFIRMÉE")
    print("="*70)
