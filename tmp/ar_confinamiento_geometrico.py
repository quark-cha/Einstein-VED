...
def crear_animacion_confinamiento():
    ...
    line, = ax.plot([], [], 'b-', linewidth=3, label='موجة ψ(x)')
    
    # Ejes y etiquetas iniciales
    ax.set_xlabel('الموضع x', fontsize=12)
    ax.set_ylabel('السعة ψ(x)', fontsize=12)
    ax.set_title('الحصر أينشتاين-VED: موجة → جسيم', fontweight='bold', fontsize=14)
    ...
    def animate(frame):
        ...
        if frame < 30:  # FASE 1: Onda completa en papel
            ...
            ax.set_xlabel('الموضع x', fontsize=12)
            ax.set_ylabel('السعة ψ(x)', fontsize=12)
            ax.set_title('1. موجة جيبية على سطح مستو', fontweight='bold')
            ...
            ax.set_title(f'2. انتقال: التفاف ({progress*100:.0f}%)', fontweight='bold')
            ax.set_xlabel('الإحداثي X')
            ax.set_ylabel('الإحداثي Y')
            ...
            ax.plot(x_wave, y_wave, 'red', linewidth=3, label=f'محصورة (n={n_cycles})')
            ax.text(0.4, -1.8, f'شرط أينشتاين-VED: L = {n_cycles}λ', 
                    fontweight='bold', fontsize=12,
                    bbox=dict(boxstyle="round", facecolor="yellow"))
            ax.set_title('3. موجة ذاتية الحصر - جسيم مستقر', fontweight='bold')
            ax.set_xlabel('الإحداثي X')
            ax.set_ylabel('الإحداثي Y')
            ax.legend()
...
def generar_resumen_confinamiento():
    ...
    ax1.set_title('أ) موجة حرة مستوية', fontweight='bold')
    ax1.set_xlabel('الموضع x')
    ax1.set_ylabel('السعة ψ(x)')
    ...
    ax2.set_title('ب) انتقال', fontweight='bold')
    ax2.set_xlabel('الإحداثي X')
    ax2.set_ylabel('الإحداثي Y')
    ...
    ax3.set_title('ج) محصورة n=3\nL = 3λ', fontweight='bold')
    ax3.set_xlabel('الإحداثي X')
    ax3.set_ylabel('الإحداثي Y')
    ax3.grid(True, alpha=0.3)
    ax3.text(1.6, -1.3, 'الشرط: L = nλ', ha='center', fontweight='bold',
             bbox=dict(boxstyle="round", facecolor="yellow"))

    plt.tight_layout()
    plt.savefig(r"C:\Users\vedq\Desktop\desarrollo\SRC-VED\Einstein-VED\img\AR_confinamiento.svg", format='svg', bbox_inches='tight')
    print("SVG resumen guardado: 'resumen_confinamiento.svg'")

# Ejecutar
print("جاري توليد رسوم متحركة SVG للحصر...")
crear_animacion_confinamiento()
generar_resumen_confinamiento()
print("✅ اكتملت الرسوم المتحركة SVG")
